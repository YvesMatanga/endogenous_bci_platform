function [modelParametersSession_x,statsSession_x,dxySession,...
    ...
    targetSession,modelParametersSession_y,statsSession_y,FeaturesSession_y] = getDataFromFrameSvr(sim_out,MaxSvs,dim)
...
FeaturesSession_y = cell(N,1);
 ...
 if dim == 1
NFr = ((NCs - 8)/(MaxSvs+1)) - 1
NSvrCoeffs = (NFr+1)*MaxSvs + 2
for i=1:N 
modelParametersSession_x{i,:} = unique(sim_out(t0Ids(i):t1Ids(i),1:NSvrCoeffs),'rows');
statsSession_x{i,:} = unique(sim_out(t0Ids(i):t1Ids(i),NSvrCoeffs+1:NSvrCoeffs+2),'rows');
dxySession{i,:} = sim_out(t0Ids(i):t1Ids(i),NSvrCoeffs+3:NSvrCoeffs+4);
dxyRawSession{i,:} = sim_out(t0Ids(i):t1Ids(i),NSvrCoeffs+5:NSvrCoeffs+6);
FeaturesSession_x{i,:} = sim_out(t0Ids(i):t1Ids(i),NSvrCoeffs+7:NSvrCoeffs+7+NFr-1);
targetSession{i,:} = unique(sim_out(t0Ids(i):t1Ids(i),NSvrCoeffs+7+NFr),'rows');
end
