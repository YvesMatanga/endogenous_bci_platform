function [model, time, resultsEval] = mars_build(Xtr, Ytr, trainParams, ...
...
%                   function areseq to simplify equations.
...
if trainParams.maxFuncs >= 0
    ...
    if verbose, fprintf('Setting trainParams.maxFuncs to %f\n', trainParams.maxFuncs); end
end
...
if (trainParams.c < 0)
    ...
    if verbose, fprintf('Setting trainParams.c to %f\n', trainParams.c); end
end
...
if isempty(modelOld)
    params = aresparamsInit();%added code
    model = model_build(params);%added code
    X = ones(n,1);
...
%     N = 100;
%     model.knotdims = zeros(N,1);%model.knotdims = {};
%     model.knotsites = zeros(N,1);%model.knotsites = {};
%     model.knotdirs = zeros(N,1);%model.knotdirs = {};
%     model.parents = zeros(N,1);%model.parents = [];
%     model.trainParams = [];%model.trainParams = [];
%     
%     coder.varsize('model.coefs');
%     coder.varsize('model.knotdims');
%     coder.varsize('model.knotsites');
%     coder.varsize('model.knotdirs');
%     coder.varsize('model.parents');
else
    % modelOld is the initial model
    if dy == 1%binary model
        model = modelOld;
%     else
%         model = modelOld{1};
%         for k = 1 : dy
...
%             end
    ...
    if isempty(modelOld) % no forward phase when modelOld is used
        ...
        if (trainParams.fastK < Inf)
            ...
            fastParent = repmat(struct('idxStart',1,'idxEnd',1,'iterComputedAllDims',1,'bestDim',1,'bestIdxInList',1,'bestErr',0),0);
            coder.varsize('fastParent');
            fastMinErrAdj = [];
            fastIterComputedErr = [];
            fastNumBasis = 0;
        end
        ...
        for currIter = 1 : maxIters
            ...
            % count the number of knot locations for the linear basis functions
...
% functions (i.e., basis functions without without interactions). If it's
% non-empty, adds only basis functions with interactions which result from
% the last numNewFuncs basis functions (typically 2, sometimes 1).
...
if (isempty(basisFunctionList_old)) && (numNewFuncs == 0)
    ...
    basisFunctionList = zeros(4,0);
    coder.varsize('basisFunctionList');%added code
    counter = 0;
...
%made function
%function parent = parentInit()
%parent = struct('idxStart',1,'idxEnd',1,'iterComputedAllDims',1);
%return